...
| Apple／Google 登入 | AuthenticationServices、GoogleSignIn 9.2、Keychain | Simulator build；正式 token 待 OAuth／Apple Developer 設定 |
| iPhone／iPad／Web 同步 | `SyncClient`、Cloudflare Worker、Turso migration | iOS merge/outbox tests、Worker tests；正式三端 E2E 待部署 |

## 尚待正式環境驗收

- Web OAuth、資料匯出與帳號刪除。
- Worker refresh rotation、rate limit、越權與 replay 測試。
