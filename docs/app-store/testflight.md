...
| iPad | 直向、橫向、Split View、鍵盤／觸控 | build 通過；UI runner 環境仍會卡在安裝／啟動階段 |
| 離線 | 首次安裝後飛航模式、遊玩、重開 App | 本機資料庫與單元測試完成；真機待驗 |
| 同步 | 首登合併、雙端衝突、墨滴事件去重 | 單元／契約測試完成；正式三端 E2E 待部署 |
...
- [ ] 建立 Turso 資料庫、套用 migrations，部署 Cloudflare Worker 並設定 secrets／CORS。
- [ ] 將正式 Worker HTTPS 網址與 OAuth client IDs 寫入 Release build settings。
- [ ] 完成正式三端 E2E、帳號連結、權杖輪替、匯出與刪除驗收。
- [ ] 補上公開聯絡信箱、Support URL 與 Privacy Policy URL。
